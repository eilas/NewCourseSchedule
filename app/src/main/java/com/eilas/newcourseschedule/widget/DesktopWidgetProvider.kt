...
//            list view
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        RemoteViews(context?.packageName, R.layout.desktop_widget_provider).apply {
            appWidgetId?.let { setRemoteAdapter(it, R.id.desktopListView, intent1) }
        }
        val appWidgetIds = widgetManager.getAppWidgetIds(context?.let { it1 ->
            ComponentName(it1, DesktopWidgetProvider::class.java)
        })

        Log.i("widget appWidgetId in \$onReceive", appWidgetIds.clone().toMutableList().toString())
        ...
        } else if (intent?.action.equals(ACTION_DATASET_UPDATE)) {
